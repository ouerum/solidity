...
bool CHC::visit(WhileStatement const& _while)
{
	...
	auto loopHeaderBlock = createBlock(&_while, "loop_header_");
	auto loopBodyBlock = createBlock(&_while.body(), "loop_body_");
...
bool CHC::visit(ForStatement const& _for)
{
	...
	auto loopHeaderBlock = createBlock(&_for, "loop_header_");
	auto loopBodyBlock = createBlock(&_for.body(), "loop_body_");
	auto afterLoopBlock = createBlock(&functionBody);
	auto postLoop = _for.loopExpression();
	auto postLoopBlock = postLoop ? createBlock(postLoop, "loop_post_") : nullptr;
